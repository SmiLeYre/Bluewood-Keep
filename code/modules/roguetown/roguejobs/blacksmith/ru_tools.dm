...
/obj/item/rogueweapon/hammer
	...
	name = "молот"
	desc = "Каждый удар отдается громким эхом, скандирующим «В бой!»"
	icon_state = "hammer"
...
/obj/item/rogueweapon/hammer/attack_obj(obj/attacked_object, mob/living/user)
	...
	if(isbodypart(attacked_object) && !user.cmode)
		...
		if(repair_percent)
			...
			if(repair_percent == 0.01) // If an inexperienced repair attempt has been successful
				to_chat(user, span_warning("Вы с трудом пытаетесь починить [attacked_prosthetic]."))
			else	
				user.visible_message(span_info("[user] чинит [attacked_prosthetic]!"))
			...
			user.visible_message(span_warning("[user] бьет [attacked_prosthetic]!"))
	...
	if(isitem(attacked_object) && !user.cmode)
		...
		if(attacked_item.obj_integrity <= 0)
			user.visible_message(span_warning("[attacked_item] сломался ! Я не могу это починить ..."))
			return
		...
		if(repair_percent)
			...
			if(repair_percent == 0.01) // If an inexperienced repair attempt has been successful
				to_chat(user, span_warning("Вы с трудом пытаетесь починить [attacked_item]."))
			else	
			...
			user.visible_message(span_warning("[user] бьет [attacked_item]!"))
	...
	if(isstructure(attacked_object) && !user.cmode)
		...
		playsound(src,'sound/items/bsmithfail.ogg', 100, FALSE)
		user.visible_message(span_info("[user] чинит [attacked_structure]!"))
		return
...
/obj/item/rogueweapon/hammer/wood
	...
	desc = "Деревянная киянка - второй лучший друг изобретателя! Но она может пригодиться и кузнецу..."
	icon_state = "whammer"
	force = 16
	smeltresult = null
...
/obj/item/rogueweapon/tongs
	...
	name = "клешни"
	desc = "Железная клешня, используемая для переноски горячих слитков. "
	icon_state = "tongs"
	icon = 'icons/roguetown/weapons/tools.dmi'
	sharpness = IS_BLUNT
	//dropshrink = 0.8
...
/obj/item/rogueweapon/tongs/examine(mob/user)
	. = ..()
	if(hott)
		. += span_warning("Накоенчник горячий на ощупь")

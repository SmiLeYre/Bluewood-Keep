...
/datum/anvil_recipe/proc/advance(mob/user, breakthrough = FALSE)
	if(progress == 100)
		...
		user.visible_message(span_warning("[user] бьёт по заготовке !"))
		return FALSE
	if(needed_item)
		to_chat(user, span_info("Время добавить \a [initial(needed_item.name)]."))
		user.visible_message(span_warning("[user] бьёт по заготовке !"))
		return FALSE
	var/moveup = 1
	var/proab = 3
	...
	if(!moveup)
		if(prob(round(proab/2)))
			...
			user.visible_message(span_warning("[user] возится с заготовкой !"))
			return FALSE
	else
		if(user.mind)
			if(isliving(user))
		...
		if(breakthrough)
			...
			user.visible_message(span_info("[user] бьёт по заготовке !"))
		return TRUE

/obj/machinery/light/rogue/smelter
	...
	name = "каменная печь"
	desc = "Каменная печь, выветрившаяся от времени и жары."
...
/obj/machinery/light/rogue/smelter/attackby(obj/item/W, mob/living/user, params)
	if(istype(W, /obj/item/rogueweapon/tongs))
		...
		if(ore.len && !T.hingot)
			...
			I.forceMove(T)
			T.hingot = I
			user.visible_message(span_info("[user] извлекает [I] из [src]."))
			if(on)
				var/tyme = world.time
				T.hott = tyme
	...
	if((ore.len < maxore) && W.smeltresult)
		W.forceMove(src)
		ore += W
		user.visible_message("<span class='warning'>[user] кладет что-то в плавильную печку.</span>")
		cooking = 0
		return
	else
		...
		if(ore.len)
			if(cooking < 30)
				...
				if(cooking == 30)
					...
					playsound(src,'sound/misc/smelter_fin.ogg', 100, FALSE)
					visible_message(span_notice("[src] законченно."))
					cooking = 31

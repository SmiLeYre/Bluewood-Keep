...
/obj/machinery/anvil
	...
	desc = "Её поверхность изуродована бесчисленными ударами молотка."
	icon_state = "anvil"
	var/hott = null
...
/obj/machinery/anvil/examine(mob/user)
	. = ..()
	if(hingot && hott)
		. += span_warning("[hingot] слишком горячий, что бы его трогать!")
...
/obj/machinery/anvil/attackby(obj/item/W, mob/living/user, params)
	...
	if(W.anvilrepair)
		user.visible_message(span_info("[user] поставил [W] на наковальню."))
		W.forceMove(src.loc)
		return
	..()
...
/obj/machinery/anvil/proc/choose_recipe(user)
	...
	var/i_type_choice = input(user, "Выбери тип", "Наковальни") as null|anything in valid_types
	if(!i_type_choice)
		return
	...
	if(appro_recipe.len)
		var/datum/chosen_recipe = input(user, "Выбери творение", "наковальня") as null|anything in sortNames(appro_recipe.Copy())

/obj/item/fishingrod
	...
	name = "рыболовная удочка"
	desc = "Сделан из обветренного дерева и грубой бечевки. Орудие борьбы с темными водами внизу."
	icon_state = "rod"
...
/obj/item/fishingrod/attackby(obj/item/I, mob/user, params)
	if(I.baitchance && !baited)
		user.visible_message(span_notice("[user] приклепляет что-то к крючку."), \
							span_notice("Я приклепляю [I] к крючку моей удочки."))
...
/obj/item/fishingrod/afterattack(obj/target, mob/user, proximity)
	...
	current_fisherman.visible_message(span_warning("[current_fisherman] закидывает удочку!"), \
						span_notice("Я закинул удочку."))
	...
	if(!prob(baited.baitchance))
		to_chat(current_fisherman, span_warning("Черт, сбежал..."))
		QDEL_NULL(baited)
		update_icon()
		return

	to_chat(current_fisherman, span_notice("Что-то дёргает мою леску!"))
	playsound(loc, 'sound/items/fishing_plouf.ogg', 100, TRUE)
...
/obj/item/fishingrod/attack_turf(turf/T, mob/living/user)
	...
	for(var/obj/item/I in T.contents)
		if(I.baitchance)
			baitables += I
		if(baited)
			to_chat(user, "На моей удочке уже есть приманка.")
			return
		for(I in baitables)
			if(I.baitchance && !baited)
				to_chat(user, span_info("I begin baiting \the [src]..."))  /// ПЕРЕВЕСТИ !!!!

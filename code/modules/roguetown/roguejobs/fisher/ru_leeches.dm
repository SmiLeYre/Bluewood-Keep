...
/obj/item/natural/worms/leech/examine(mob/user)
	...
	switch(blood_storage/blood_maximum)
		if(0.8 to INFINITY)
			. += span_bloody("<B>[p_theyre(TRUE)] толстая и переполненная кровью.</B>")
		if(0.5 to 0.8)
			. += span_bloody("[p_theyre(TRUE)] сытая.")
		if(0.1 to 0.5)
			. += span_warning("[p_they(TRUE)] want[p_s()] хочет есть.")
		if(-INFINITY to 0.1)
			. += span_dead("[p_theyre(TRUE)] мертвая.")
	if(!giving)
		. += span_warning("[p_theyre(TRUE)] [pick("слюнявит", "сосёт", "вдыхает")].")
	else
		. += span_notice("[p_theyre(TRUE)] [pick("тошнит", "принюхивается", "выдыхает")].")
	if(drainage)
		START_PROCESSING(SSobj, src)
...
/obj/item/natural/worms/leech/attack(mob/living/M, mob/user)
	if(ishuman(M))
		...
		if(completely_silent)
			return
		if(M == user)
			user.visible_message(span_notice("[user] прикладывает [src] на [user.p_their()] [affecting]."), span_notice("Я прикладываю пиявку к [affecting]."))
		else
...
/obj/item/natural/worms/leech/proc/leech_lore()
	...
	var/static/list/all_descs = list(
		"Что за мерзкое создание." = 10,
		"Ебаная гадость." = 5,
		"Скользкая..." = 3,
		"Такая аппетитная и полная крови." = 3,
		"Мне нравится эта пиявка!" = 2,
		"Она такая прекрасная." = 2,
		"Я бы хотел быть пиявкой." = 1,
	)
...
/obj/item/natural/worms/leech/cheele
	name = "чиили"
	desc = "Прекрасный, наполненный кровью альтруистический организм, созданный самой Пестрой."
	icon_state = "cheele"
	color = null
	consistent = TRUE
	drainage = 0
...
/obj/item/natural/worms/leech/cheele/attack_self(mob/user)
	...
	if(giving)
		user.visible_message(span_notice("[user] сжимает [src]."),\
							span_notice("Я сжимаю [src]. Она теперь будет вливать кровь."))
	else
		user.visible_message(span_notice("[user] сжимает [src]."),\
							span_notice("Я сжимаю [src]. Она теперь будет высасывать кровь."))

/obj/item/natural/worms/leech/propaganda
	name = "проклятая пиявка"
	desc = "Пиявка, которой нет равных."
	drainage = 0
	blood_sucking = 0
...
/obj/item/natural/worms/leech/propaganda/on_embed_life(mob/living/user, obj/item/bodypart/bodypart)
	...
	if(iscarbon(user))
		...
		if(prob(5))
			V.say(pick("СЛАВА ЗИЗО !", "СМЕРТЬ ДЕСЯТКЕ ..."))

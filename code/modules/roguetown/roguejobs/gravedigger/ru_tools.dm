/obj/item/rogueweapon/shovel
	...
	name = "лопата"
	desc = "Необходим для рытья могил в этой мрачной земле."
	icon_state = "shovel"
...
//				T.ChangeTurf(/turf/open/floor/rogue/dirt/road, flags = CHANGETURF_INHERIT_AIR)
		...
		if(istype(T, /turf/open/floor/rogue/grass))
			to_chat(user, span_warning("Трава мешает."))
			return
		return
	. = ..()
...
/obj/item/rogueweapon/shovel/small
	force = 7
	name = "лопатка"
	desc = "Незаменим для ухода за почвой."
	icon_state = "spade"
	sharpness = IS_BLUNT
	//dropshrink = 0.8
...
/obj/item/burial_shroud
	name = "погребальная пелена"
	desc = "Погребальная пелена для покойного."
...
/obj/structure/closet/burial_shroud/MouseDrop(over_object, src_location, over_location)
	...
	if(over_object == usr && Adjacent(usr) && (in_range(src, usr) || usr.contents.Find(src)))
		if(!ishuman(usr))
			return
		if(contents.len)
			to_chat(usr, span_warning("Внутри [src] слишком много вещей, что бы его закрыть!"))
			return
		visible_message(span_notice("[usr] закрывает [src]."))

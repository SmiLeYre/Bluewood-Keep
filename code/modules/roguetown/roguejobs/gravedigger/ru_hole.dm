...
/obj/structure/closet/dirthole/grave
	desc = "Отверстие достаточно большое для гроба."
	stage = 3
	faildirt = 3
	icon_state = "grave"

/obj/structure/closet/dirthole/closed
	desc = "Гора грязи, под которой кто-то лежит."
	stage = 4
	faildirt = 3
	climb_offset = 10
...
/obj/structure/closet/dirthole/closed/loot/examine(mob/user)
	...
	if(HAS_TRAIT(user, TRAIT_SOUL_EXAMINE))
		if(lootroll == 1)
			. += span_warning("Лучше, пусть оно спит..")
...
//								return
			to_chat(user, span_warning("Я не могу зарыться еще глубже.."))
			return
		var/used_str = 10
		if(iscarbon(user))

...
/obj/machinery/light/rogue/oven
	...
	name = "каменая печь"
	desc = "Стоит на страже от голода и темноты, когда горит."
	icon_state = "oven1"
	base_state = "oven"
	density = FALSE
	on = FALSE
...
/obj/machinery/light/rogue/oven/attackby(obj/item/W, mob/living/user, params)
	...
	if(clicked_top)
		...
		if(food.len < maxfood)
			...
			user.visible_message(span_warning("[user] ставит что-то в печь."))
	...
	if(on)
		for(var/obj/item/I in food)
			...
			if(C)
				...
				qdel(I)
				food += C
				visible_message(span_notice("Что-то хорошо пахнет!"))
				need_underlay_update = TRUE
		update_icon()
